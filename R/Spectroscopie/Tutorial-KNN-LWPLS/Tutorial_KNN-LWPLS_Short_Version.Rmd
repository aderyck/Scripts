...
```{r}
## Load necessary packages (those packages need to be installed beforehand)
...
## Compute and display the RMSEP values on Val to select the optimal 
...
source(paste0(
  ...
  "R/Plotting_tools/InteractPlot4Parameters.R"
))
...
model <- lwplsr(
  Xtrain, ytrain,
  nlvdis = best_params$nlvdis,
  diss   = best_params$diss,
  h      = best_params$h,
  k      = best_params$k,
  nlv    = best_params$nlv
)
...
mse_test <- mse(pred, ytest) # set of indicators
print(mse_test)
