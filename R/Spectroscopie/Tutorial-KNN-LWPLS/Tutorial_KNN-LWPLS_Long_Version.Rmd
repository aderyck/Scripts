...
source("https://raw.githubusercontent.com/aderyck/Functions/main/R/Plotting_tools/InteractPlot4Parameters.R") # function for interactive plots for parameter selection, based on plotly and hosted on Github
...
X_file <- paste0(base_url, "/challenge2018_Xp.csv") # preprocessed spectra
...
## Extract train and test data (Train/Test)
...
df2d <- as_tibble(scores) %>% select(PC1, PC2) %>% mutate(Dataset = factor(ifelse(s, "1-Test", "0-Train")))

ggplot(df2d, aes(x = PC1, y = PC2, color = Dataset)) + geom_point(size = 1.5, alpha = 0.8) + scale_color_manual(values = c("0-Train" = "blue", "1-Test" = "red")) + labs(x = "PC1", y = "PC2", color = "Dataset") + guides(color = guide_legend(override.aes = list(size = 2, alpha = 1))) + theme_bw() + theme_bw() + theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_blank(), legend.title = element_text(size = 14), legend.text = element_text(size = 12), legend.key.size = unit(1.2, "lines"))
...
df_dist <- rbind(data.frame(SD = sd_train, OD = od_train, Dataset = "0-Train", stringsAsFactors = FALSE), data.frame(SD = sd_test, OD = od_test, Dataset = "1-Test", stringsAsFactors = FALSE))

ggplot(df_dist, aes(SD, OD, color = Dataset)) + geom_point(size = 1.5, alpha = 0.8) + scale_color_manual(values = c("0-Train" = "blue", "1-Test" = "red")) +   geom_vline(xintercept = sd_cutoff, linetype = "dashed") + geom_hline(yintercept = od_cutoff, linetype = "dashed") + labs(x = "SD", y = "OD", color = "Dataset") + guides(color = guide_legend(override.aes = list(size = 2, alpha = 1))) + theme_bw() + theme(panel.grid.major = element_line(color = "grey80"), panel.grid.minor = element_blank(), legend.title = element_text(size = 14), legend.text = element_text(size = 12), legend.key.size = unit(1.2, "lines"))
...
dfh <- data.frame(Protein = y, Dataset = factor(ifelse(s, "1-Test", "0-Train")))

ggplot(dfh, aes(x = Protein, fill = Dataset)) + geom_histogram(data  = subset(dfh, Dataset == "0-Train"), bins  = 30, color = "grey95", size = 0.05, alpha = 1) + geom_histogram(data  = subset(dfh, Dataset == "1-Test"), bins  = 30, color = "grey95", size  = 0.05, alpha = 1) + scale_fill_manual(values = c("0-Train" = "steelblue", "1-Test" = "goldenrod")) + labs(x = "Protein", y    = "Nb. observations", fill = "") + theme_bw() + theme(legend.position = "top", axis.title = element_text(size = 12), axis.text = element_text(size = 10), legend.text = element_text(size = 11), panel.grid.major  = element_line(color = "grey80"), panel.grid.minor  = element_blank())
```

Model tuning
```{r}
## Split Train into calibration and validation data (Cal/Val)
nval   <- 300
...
?lwplsr # display help-page of the function and parameters’ descriptions
...
nrow(pars) # number of parameter combinations considered

## Compute and display the RMSEP values on Val to select the optimal combination 
## of parameters
...
Final predictions
```{r}
## Extract best combination of parameters
best_params <- res[which.min(res$y1), ]
best_params

## Define model with selected parameters
model <- lwplsr(Xtrain, ytrain, nlvdis = best_params$nlvdis, diss = best_params$diss, h = best_params$h, k = best_params$k, nlv = best_params$nlv)
...
mse <- mse(pred, ytest) # # set of indicators for regression, returns misclassification rate if categorical variable
print(mse)
